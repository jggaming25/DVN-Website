...
function getTransporter() {
  ...
  if (!process.env.SMTP_USER || process.env.SMTP_USER === 'deine-email@domain.de') return null;
  ...
  try {
    await transport.sendMail({
      ...
      html: `<div style="font-family:sans-serif;max-width:500px;margin:auto;padding:20px;background:#f8f9fc;border-radius:12px">
        <div style="background:linear-gradient(135deg,#c62828,#8e0000);color:#fff;padding:20px;border-radius:12px 12px 0 0;text-align:center">
          <h2 style="margin:0">Passwort-Aenderungsanfrage</h2>
        </div>
...
async function sendTicketNotification(ticketNumber, subject, author, content) {
  ...
  try {
    await transport.sendMail({
      ...
      html: `<div style="font-family:sans-serif;max-width:500px;margin:auto;padding:20px;background:#f8f9fc;border-radius:12px">
        <div style="background:linear-gradient(135deg,#c62828,#8e0000);color:#fff;padding:20px;border-radius:12px 12px 0 0;text-align:center">
          <h2 style="margin:0">Neue Ticket-Nachricht</h2>
        </div>
        <div style="background:#fff;padding:20px;border-radius:0 0 12px 12px;border:1px solid #e8ecf4">
...
module.exports = { sendPasswordRequest, sendTicketNotification };
